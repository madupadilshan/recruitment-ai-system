...
// Modern notification banner component
const NotificationBanner = ({ notification, onDismiss }) => {
  ...
  const getStyles = () => {
    const baseStyles = "fixed top-4 left-1/2 transform -translate-x-1/2 z-50 max-w-md w-full mx-4 rounded-lg shadow-lg border-l-4 p-4 transition-all duration-300";
    ...
    const typeStyles = {
      ...
      loading: "bg-gray-50 border-gray-400 text-gray-800"
    };
              ...
              {actions.map((action, index) => (
                <button
                  ...
                  className={`text-sm font-medium hover:underline ${
                    action.primary 
                      ? 'bg-white px-3 py-1 rounded-md shadow-sm hover:bg-gray-50' 
                      : 'underline'
                  }`}
                >
                  {action.label}
...
export const NotificationProvider = ({ children }) => {
  ...
  const showError = useCallback((message, options = {}) => {
    showNotification({ type: 'error', message, duration: 7000, ...options });
  }, [showNotification]);
  ...
  const showLoading = useCallback((message, options = {}) => {
    showNotification({ type: 'loading', message, duration: 0, ...options });
  }, [showNotification]);
